  ...
  const takeAction = async (action) => {
    try {
      ...
      await firebase
        ...
        .doc(userId)
        .update({
          [`${action}s`]: firebase.firestore.FieldValue.arrayUnion(
            ratedPerson.id
          ),
        });
      ...
      if (action === "like" || action === "favorite") {
        try {
          const ratedPersonDocument = await (
            await firebase
              .firestore()
              .collection("/users")
              .doc(ratedPerson.id)
          ...
          if (
            ratedPersonDocument.likes.includes(userId) ||
      ...
      {personsArray.length > 0 && (
        <div className="flex flex-col items-center">
          ...
          >
            ...
            <p className="absolute bottom-0 group-hover:top-0 m-4 pb-0 md:pb-5 text-white text-lg font-bold transition-all">
              {currentPerson.name}
            </p>
            <p className="hidden md:block absolute bottom-0 group-hover:top-0 m-4 pt-6 pr-8 text-white text-md truncate group-hover:whitespace-normal transition-all w-full">
              {currentPerson.description}
            </p>
          </div>
          <p className="block md:hidden text-gray-500 my-2 text-center">
            {currentPerson.description}
          </p>
